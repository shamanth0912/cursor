...
(function () {
  ...
  try {
    document.documentElement.classList.remove("layout-classic", "layout-current");
  ...
  function paintBlend(t) {
    ...
    if (isMobile()) {
      ...
      var titleH = Math.round(h * 0.14);
      var bandBottom = Math.round(h * 0.34);
      var photoTop = navH + titleH + 8;
      var photoH = Math.max(120, h - photoTop - bandBottom);
      var photoPad = Math.round(w * 0.04);
      drawCoverBox(frames[i0], photoPad, photoTop, w - photoPad * 2, photoH, vis);
    } else if (LAYOUT === "classic") {
      /* Classic 2D fallback: framed plate on the right, solid ink left for copy. */
      var railW = Math.round(Math.min(w * 0.36, 420));
      var platePad = Math.round(Math.min(w, h) * 0.04);
      var plateX = railW + platePad;
      var plateY = Math.round(h * 0.12);
      var plateW = Math.max(160, w - plateX - platePad);
      var plateH = Math.max(160, h - plateY - Math.round(h * 0.1));
      drawCoverBox(frames[i0], plateX, plateY, plateW, plateH, vis);
    } else {
      ctx.save();
